...
ARGPARSE_DESCRIPTION="Add new Varbase 9.0.x-dev ready Automated testing builds, and install. Then run tests using a real browser or headless browser"
argparse "$@" <<EOF || exit 1
...
composer require --dev drush/drush:~12.0 --with-all-dependencies;
...
sed -i "s,/var/www/html/test/varbase/docroot,${vdo_root}/${doc_name}/${PROJECT_NAME}/docroot,g" ${vdo_root}/${doc_name}/${PROJECT_NAME}/docroot/profiles/varbase/behat.yml ;
